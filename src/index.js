...
class DaoApi {
    ...
    async run() {
        ...
        while(this.DaoRegistrar.har.length)
        {
            ...
            _data.data.data.creator=_info[1]
        ...
        while(this.UnitToken.e2uAr.length)
        {
            ...
            let eventData=await utils.getCheckEvent(this.ethersProvider,this.eventIface,'ETHToSCToken',_data.data.transactionHash)    
            if(!eventData.isOk) {
         ...
         while(this.IADD.t2uAr.length)
         {
             ...
             let eventData=await utils.getCheckEvent(this.ethersProvider,this.eventIface,'SCTokenToSCTokenEvent', _data.data.transactionHash)    
             if(!eventData.isOk)
             {
                _data.data.data.gas=eventData.gasUsed
                _data.data.data.swap_time=await utils.getTime(this.web3,_data.data.blockNumber)
         ...
         while(this.IADD.u2tAr.length)
         {
             ...
             let eventData=await utils.getCheckEvent(this.ethersProvider,this.eventIface,'ETHToSCToken', _data.data.transactionHash)    
             if(!eventData.isOk) eventData=await utils.getCheckEvent(this.ethersProvider,this.eventIface,'SCTokenToSCTokenEvent', _data.data.transactionHash)    
             if(!eventData.isOk)
             {
                _data.data.data.gas=eventData.gasUsed
                _data.data.data.swap_time=await utils.getTime(this.web3,_data.data.blockNumber)
    ...
    get ethersProvider()
    {
        ...
        this.ethersProvider_obj= new ethers.JsonRpcProvider(process.env.HTTPS_URL)
